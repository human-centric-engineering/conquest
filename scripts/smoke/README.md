...
| `epub-ingestion.ts`      | Real `.epub` upload → `parseEpub` → chunk → store, via `POST /knowledge/documents` on a running server                                                                                                                                                                                                 | Nothing — that is the point                                                                                                                 | **Point it at a production build** (`npm run build && PORT=3100 npm start`, then `SMOKE_BASE_URL=http://localhost:3100`). The EPUB path has broken twice in ways vitest could not see (#606; jsdom's ESM move before it), and it now carries both risks at once — extraction goes through jsdom, and `epub` is ESM-only. Signs up a `smoke-test-epub` admin, verifies it in-DB (production requires email verification), and removes both rows. |
...
| `sectioned-interview.ts` | Sectioned interviews (P21) end to end: the resolver ladder, the two-section floor, the turn-context bound (`sectionQuestions` vs `questions`), the transcript's section headings, the report's chapters, and the admin section timeline                                                                | LLM never invoked — every seam exercised is a loader or a pure builder                                                                      | The valuable assertions are the NEGATIVE ones: with the feature off the turn context carries no section pools at all, a one-section version runs unsectioned, and switching the setting off unsections a session that already banked a run. Everything hangs off one `__sections_smoke__` questionnaire; cleanup cascades the whole graph.                                                                                                      |
