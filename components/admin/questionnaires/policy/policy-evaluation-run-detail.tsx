...
import type {
  ...
  PolicyEvaluationRunDetail as PolicyEvaluationRunDetailView,
} from '@/lib/app/questionnaire/views';
import { PolicyFindingReviewCard } from '@/components/admin/questionnaires/policy/policy-finding-review-card';
import {
  JudgeFailureIcon,
  RetryJudgeButton,
  judgeFailureReason,
...
interface Props {
  run: PolicyEvaluationRunDetailView;
...
export function PolicyEvaluationRunDetail({ run, questionnaireId, versionId, canApply }: Props) {
  const [current, setCurrent] = useState(run);
  const [retrying, setRetrying] = useState<PolicyEvaluationDimension | null>(null);
  const [retryError, setRetryError] = useState<string | null>(null);
  const [forkNotice, setForkNotice] = useState<number | null>(null);

  const severity = tallyPolicySeverities(current.findings);
  const reviewed = current.findings.filter((f) => f.status !== 'pending').length;
  const pending = current.findings.length - reviewed;
  const groups = groupPolicyFindingsByTarget(current.findings);
  ...
  function updateFinding(
    next: PolicyEvaluationFindingView,
  ...
  async function retryJudge(dimension: string) {
    const dim = dimension as PolicyEvaluationDimension;
    setRetrying(dim);
    setRetryError(null);
    try {
      const res = await fetch(
        API.APP.QUESTIONNAIRES.versionPolicyEvaluationRetryJudge(
          questionnaireId,
          versionId,
          run.id
        ),
      ...
      <div className="cq-rise bg-card rounded-xl border p-4">
        ...
        <div className="grid [grid-template-columns:repeat(auto-fit,minmax(160px,1fr))] gap-2">
          {current.dimensionSummary.map((dim) => {
            const spec = POLICY_EVALUATION_DIMENSION_SPECS[dim.dimension];
