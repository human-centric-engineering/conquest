...
/**
 * PolicyEvaluationRunsTable (F17.21) — the Conditional Topics evaluation run history for one version.
 *
...
interface Props {
  ...
  runs: PolicyEvaluationRunListItem[];
  canRun: boolean;
}

export function PolicyEvaluationRunsTable({
  questionnaireId,
  versionId,
  versionNumber,
  runs,
  ...
  const detailHref = (runId: string): string =>
    `/admin/questionnaires/${questionnaireId}/v/${versionId}/settings/policy-evaluations/${runId}`;
  ...
  const runEvaluation = async () => {
    ...
    try {
      const res = await fetch(
        API.APP.QUESTIONNAIRES.versionPolicyEvaluations(questionnaireId, versionId),
        {
          method: 'POST',
      ...
      const body = await parseApiResponse<PolicyEvaluationRunDetail>(res);
