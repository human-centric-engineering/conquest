...
/**
 * Fold state for the settings groups, shared by every `SettingsGroup` on the tab.
 *
 * Provided by the editor rather than threaded through fifteen call sites: a group only needs to
 * know whether *its* id is folded, and jumping to a section (from the rail or from a conflict's
 * `#anchor`) has to be able to unfold it from the outside. Absent provider → everything is open,
 * which keeps the component usable on its own.
 */
interface SettingsCollapseApi {
  /** Section ids currently folded shut. */
  collapsed: string[];
  toggle: (id: string) => void;
  expand: (id: string) => void;
}

const SettingsCollapseContext = createContext<SettingsCollapseApi | null>(null);
...
function SettingsGroup({
  ...
  const collapseApi = useContext(SettingsCollapseContext);
  const open = !collapseApi?.collapsed.includes(id);
  const expand = collapseApi?.expand;

  // A conflict row (or any pasted link) navigating to `#this-section` must unfold it — landing on
  // a shut card would look like the setting had vanished. Covers the load-with-hash case too.
  useEffect(() => {
    if (!expand) return;
    const openIfTargeted = () => {
      if (window.location.hash === `#${id}`) expand(id);
    };
    openIfTargeted();
      ...
      >
        ...
        {collapseApi && (
          <Button
            ...
            variant="ghost"
            size="icon"
            className="text-muted-foreground hover:text-foreground -mr-1 h-7 w-7 shrink-0"
            aria-expanded={open}
...
export function ConfigEditor({
   ...
   *
   * Topics, criteria, the budget and every other scope setting stay on the Conditional topics tab,
   ...
   */
  const [conditionalTopicsEnabled, setConditionalTopicsEnabled] = useState(
    config.conditionalTopics.enabled
  );
  ...
  const conflicts = useMemo(
    () =>
      detectConfigConflicts({
        ...
        // an admin turning conditional topics on should see the strategy warnings it raises while
  ...
  /* ── Fold state for the settings groups ─────────────────────────────────────────────────────
     Fifteen groups is more than a screen of headings, so each one folds and the choice sticks
     per browser — an admin who only ever touches tone and access shouldn't scroll past the rest
     on every visit. Ids are read from the DOM for "collapse all" (same `[data-section-rail]`
     discovery the rail uses), so a new group needs no list kept in sync. */
  const [collapsedSections, setCollapsedSections] = useLocalStorage<string[]>(
    'cq.questionnaire.settings.collapsed.v1',
    []
  );
  ...
  const expandSection = useCallback(
    (id: string) =>
      setCollapsedSections((prev) => (prev.includes(id) ? prev.filter((s) => s !== id) : prev)),
    [setCollapsedSections]
  );
  const collapseApi = useMemo(
    () => ({ collapsed: collapsedSections, toggle: toggleSection, expand: expandSection }),
    [collapsedSections, toggleSection, expandSection]
  );
  /* Collapse-all reads the rendered ids in the click handler — never during render, where there
     is no `document` on the server pass. Anything folded flips the control to "Expand all". */
  const collapseAll = () =>
    setCollapsedSections(
      Array.from(
        document
          .getElementById('settings-sections')
          ?.querySelectorAll<HTMLElement>('[data-section-rail]') ?? []
      )
        .map((el) => el.id)
        .filter(Boolean)
    );
  const somethingFolded = collapsedSections.length > 0;
    ...
    run(() => [
      ...
      API.APP.QUESTIONNAIRES.versionConfig(questionnaireId, versionId),
      {
        ...
        profileFields: captureEnabled
          ...
          : [],
        // Conditional Topics' master switch — sent ONLY when this editor actually changed it.
        //
        // Every other field here has exactly one editor, so sending it unconditionally can only
        // ever rewrite it with itself. This one is also on the Conditional topics tab: an admin who
        ...
        //
        // The route merges `conditionalTopics` through the same helper the Topics tab's own PATCH
        ...
        ...(conditionalTopicsEnabled !== config.conditionalTopics.enabled
          ? { conditionalTopics: { enabled: conditionalTopicsEnabled } }
          : {}),
      },
    ]);
  ...
  return (
    <section className="space-y-4">
      ...
      <SettingsCollapseContext.Provider value={collapseApi}>
          ...
          <div id="settings-sections" className="min-w-0 space-y-4 lg:col-start-2">
            ...
            <div className="flex justify-end">
              <Button
                ...
                onClick={() => (somethingFolded ? setCollapsedSections([]) : collapseAll())}
              >
                {somethingFolded ? 'Expand all' : 'Collapse all'}
              </Button>
            </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Label htmlFor="question-fidelity-enabled" className="text-sm font-medium">
                    ...
                    <FieldHelp title="Question fidelity">
                      ...
                      level on the <em>Structure</em> tab. While this is off, every question is
                      asked openly and the levels you set are ignored, so you can prepare them
                  ...
                  <div className="space-y-1.5 pt-1 sm:max-w-sm">
                    <Label className="text-sm font-medium">
                      ...
                      <FieldHelp title="Level for new questions">
                        ...
                        <strong> Close</strong> as its baseline rather than{' '}
                ...
                <div className="space-y-1.5">
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Answer confidence floor">
                      ...
                      those guesses <em>tentative</em>. A tentative answer below this confidence
                      does NOT count toward completion or satisfy a required question until the
                      respondent corroborates it (each confirmation strengthens it). Lower = accept
                      guesses sooner (faster, less checking); higher = insist on firmer confirmation
                      before the form is &ldquo;done&rdquo;. <code className="text-xs">0.5</code>{' '}
                      gates the agent&rsquo;s opportunistic guesses without holding back genuine
                      answers; raise toward <code className="text-xs">0.65</code>+ to demand firmer
                      confirmation.
                    </FieldHelp>
                  </Label>
                  <Input
              ...
              <div className="space-y-1.5 sm:max-w-xs">
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Data-slot attempts">
                    ...
                    guess and moves on — so a respondent never gets stuck being asked the same
                    thing. <code className="text-xs">1</code> = ask once and move on;{' '}
                    ...
                    only-inferred answer (low confidence) is the kind the agent circles back on, and
                    a higher value lets it probe such answers further at the cost of a longer
              ...
              <div className="flex items-center gap-2">
                ...
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Let respondents finish early">
                    Adds a calm &ldquo;Continue or finish up&rdquo; control to the
                    respondent&rsquo;s screen once they&rsquo;ve answered enough. Choosing
                    &ldquo;Finish up&rdquo; ends the session and prepares their report — even if the
                    agent&rsquo;s own completion thresholds above aren&rsquo;t met, and{' '}
              ...
              {allowEarlyFinish && (
                <div className="space-y-3">
                  ...
                  <p className="text-muted-foreground text-xs leading-relaxed">
                    ...
                    ignore that axis. The default (100% coverage, questions off) shows the button
                    only once they&rsquo;ve effectively completed the questionnaire; lower the
                    coverage to let them finish sooner.
                  </p>
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium">
            ...
            {/* ── Conditional topics — the master switch, mirrored from the Conditional topics tab so the
            ...
            <SettingsGroup
              ...
              headerAction={
                <label className="flex cursor-pointer items-center gap-2 text-xs font-medium select-none">
                  <span
                    className={
                      conditionalTopicsEnabled ? 'text-foreground' : 'text-muted-foreground'
                    }
                  >
                    {conditionalTopicsEnabled ? 'On' : 'Off'}
                  </span>
                  <Switch
                    checked={conditionalTopicsEnabled}
                    onCheckedChange={setConditionalTopicsEnabled}
                    disabled={busy}
                    aria-label="Use conditional topics"
                  />
                </label>
              }
            >
                ...
                <p className="text-muted-foreground text-sm">
                  {conditionalTopicsEnabled ? (
                    <>
                      ...
                      condition on it is still asked of everyone — so if nothing on the Conditional
                      topics tab has a condition yet, everyone is asked everything.
                    </>
                  ) : (
                    <>
                      Every respondent is asked the whole questionnaire. Anything set up on the
                      Conditional topics tab sits idle until this is on.
                    </>
                  )}{' '}
                  <FieldHelp title="Conditional topics">
                    ...
                    earlier answer went a certain way. With this on, the opening answers decide
                    which of those a respondent is asked, and the report says which parts were left
                    out and why. With it off, everyone is asked everything. Off by default, and
                    turning it on changes nothing until at least one topic has a condition written
                    on it.
                  </FieldHelp>
                </p>
              ...
              <div className="mb-4 space-y-1.5">
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Respondent layout">
                    ...
                    ConQuest Classic is the default and is what every questionnaire has always
                    looked like. Whichever you choose, no feature is lost: a layout that has no room
                    for something moves it (Focus keeps the captured answers a tap away in a sheet)
              ...
              <div className="mb-4 space-y-1.5">
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Respondent design">
                    ...
                    <br />
                    <br />
                    ConQuest Rounded is the default and is what every questionnaire has always
                    looked like. Press is straight lines and hairline rules — the register of a
                    printed report, for serious instruments and professional audiences; only the
                    answer box keeps a soft corner, because it is the one thing a respondent
                    touches. Marque is straight lines too, and puts the client&apos;s brand into the
                    structure: their logo mark signs every question the interviewer asks, an accent
                    spine runs down the conversation, and their colour closes the header.
                    <br />
                    <br />
                    Marque is at its best for a client with a strong logo mark. Without one it still
                    works — the mark beside each question falls back to a small block of their
                    accent colour — but the design has less to say.
                  </FieldHelp>
                </Label>
              ...
              <div className="mb-4 space-y-1.5">
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="ConQuest branding">
                    ...
                    join link); a respondent signed in to ConQuest keeps the app&apos;s own
                    navigation either way, since hiding it would leave them with no way out.
                  </FieldHelp>
                </Label>
                <Select
              ...
              <div className="mb-4 space-y-1.5">
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Text size">
                    ...
                    <br />
                    <br />
                    It is a starting size, not a lock. Respondents can always resize the
                    conversation from the controls beside it, and once they do, their size stands
                    until you move this setting again. You cannot take the control away from them.
                  </FieldHelp>
                </Label>
                <Select
                  value={chatTextSize}
                ...
                <p className="text-muted-foreground text-xs leading-relaxed">
                  The size the conversation opens at — respondents can still resize it, and their
                  own size then stands until you change this setting again.
                </p>
              </div>
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1.5">
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Presentation mode">
                      ...
                      right input per type (likert, choices, yes/no, text…). Both offers a chat ↔
                      form toggle so the respondent can navigate sections, see what&apos;s already
                ...
                <div className="space-y-1.5">
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Answer panel scope">
                      How much of the questionnaire the live answer panel shows the respondent
                      beside the chat. Full progress lists every question grouped by section with an
                      answered-count; answered only shows just the answers captured so far. Hidden
                      removes the panel entirely — the respondent sees only the conversation, with
                      no side panel, no &ldquo;Review answers&rdquo; sheet on mobile, and no data
                      slots on show. Independent of presentation mode: a hidden panel still allows
                      the form tab if you have one.
                    </FieldHelp>
                  </Label>
                  <Select
              ...
              <div className="flex items-center gap-2">
                ...
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Inline answer correction">
                    ...
                    editor — beneath the most-recent message in the chat and on the answer-panel
                    rows — instead of re-explaining in a new message. Corrections save directly (the
                    same path as the form view), so they don&apos;t spend a turn or trip a
                    contradiction notice. On by default.
                  </FieldHelp>
                </Label>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Resume in-progress sessions">
                    ...
                    reopening it shows a &ldquo;Continue where you left off / Start new&rdquo;
                    choice (quoting the session&apos;s reference code), and a respondent can also
                    resume from another device by entering that code. On by default. When off,
                    returning always starts a fresh session. Note: on a shared or kiosk device the
                    next person could see and continue the previous session within its 24-hour
                    window — turn this off for shared devices.
                  </FieldHelp>
                </Label>
              </div>
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Voice input">
                    ...
                    composer and tells them they can talk through their answers. When off, the mic
                    is hidden and the agent never suggests it. Also requires the platform
                    voice-input flag.
                  </FieldHelp>
                </Label>
              </div>
              ...
              {!milestoneBannerEnabled ? (
                ...
                <div className="bg-muted/20 border-border/70 rounded-xl border border-dashed px-6 py-8 text-center">
                  ...
                  <p className="text-muted-foreground mx-auto mt-1 max-w-sm text-xs">
                    ...
                    inline message (&ldquo;You&apos;re halfway through&rdquo;) at chosen
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-2">
                    <Input
                      ...
                      disabled={
                        busy || milestoneBannerThresholds.length >= MAX_MILESTONE_THRESHOLDS
                      }
                      className="w-24"
                    />
                    <Button
              ...
              <div className="flex items-center gap-2">
                ...
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Intro screen">
                    ...
                    default, so existing questionnaires are unchanged.
                  </FieldHelp>
                </Label>
              </div>
              {intro.enabled && (
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-sm font-medium">
                      ...
                      <FieldHelp title="Intro video">
                        ...
                        ); it&apos;s embedded as a privacy-enhanced player. Leave blank for no
                        video.
                      </FieldHelp>
                    </Label>
                    <Input
              ...
              <div className="flex items-center gap-2">
                ...
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Reasoning stream">
                    ...
                    answers it captured (and how confident it is), any contradictions it noticed,
                    and why it&apos;s asking the next question. It&apos;s derived from work the
                      ...
                      <div className="space-y-1.5">
                        <Label className="text-sm font-medium">
                          ...
                          <FieldHelp title="Extra dwell per reasoning step">
                            Added to the dwell for each reasoning step beyond the second, so a
                            longer summary stays open long enough to read. Total ={' '}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Label className="text-sm font-medium">
                      ...
                      <FieldHelp title="Persist the reasoning trace">
                        ...
                        resumes the session or scrolls back — and is available to you afterwards.
                        When off, it shows only on the turn as it happens; resumed or earlier turns
                        show nothing.
                      </FieldHelp>
                    </Label>
                  </div>
                ...
                <>
                      ...
                      <Label className="text-sm font-medium">
                        ...
                        <FieldHelp title="Persona">
                          <p>
                            ...
                            for example “You are an experienced, supportive career coach” or “You
                            are a concise management consultant.” Free text; keep it a sentence or
                            two. The tone sliders below still apply on top of the persona.
                          </p>
                          <p className="mt-2">
                  ...
                  <details className="group bg-muted/30 rounded-md border">
                    ...
                    <div className="text-muted-foreground px-3 pb-3 text-xs leading-relaxed">
                      ...
                      <strong className="text-foreground">Balanced</strong> dials (empathy,
                      formality, verbosity, reading complexity, humour) treat 0 as neutral — it adds
                      nothing — while − and + push toward the two opposite styles shown under each
                      slider. <strong className="text-foreground">Intensity</strong> dials
                      (mirroring, mimicry, warmth, curiosity) run low → high, so every position adds
                      a clause; switch the dial off for none. The exact clause each position injects
                      is shown live beneath its slider.
                    </div>
                  </details>
                </>
              )}
              ...
              <div className="bg-muted/20 flex items-start gap-3 rounded-lg border p-3">
                ...
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Anonymous mode">
                    ...
                    invitation-only, and a named one can still be public. When anonymous, invitees
                    are tracked only as started/completed — never linked to their answers, and{' '}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Sensitivity awareness">
                      ...
                      distress, a safeguarding concern), remembers it, and treads carefully in how
                      it phrases every later question. Best-effort awareness, not a guaranteed
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium">
                        ...
                        <FieldHelp title="Support message">
                          Shown once, gently, when a serious disclosure is detected — verbatim, so
                          it can&apos;t be reworded by the agent. Leave blank to use a standard
                          support message; or write your own, e.g. &ldquo;If anything here has been
                          difficult, support is available — you can reach our team or a helpline at
                          any time.&rdquo;
                        </FieldHelp>
                      </Label>
                      <Textarea
                    ...
                    <div className="space-y-1.5 sm:max-w-md">
                      <Label className="text-sm font-medium">
                        ...
                        <FieldHelp title="Support resource URL">
                          Optional link appended to the support message (e.g. a helpline or
                          wellbeing page). Must be a valid URL.
                        </FieldHelp>
                      </Label>
                      <Input
                        type="url"
              ...
              <div className="space-y-1.5 sm:max-w-xs">
                <Label className="text-sm font-medium">
                  ...
                  <FieldHelp title="Abuse threshold">
                    ...
                    may give before the session is automatically ended. Earlier strikes get
                    escalating warnings and the answer is set aside; the Nth ends the session.
                    Colloquial or lazy answers are tolerated. Set to{' '}
                    <code className="text-xs">0</code> to turn the gate off. Requires the platform
                    seriousness-gate flag to be on.
                  </FieldHelp>
                </Label>
                <Input
                  type="number"
                ...
                <div className="space-y-1.5 sm:max-w-sm">
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Contradiction checking">
                      Whether the agent watches for an answer that contradicts an earlier one. When
                      it is <strong>on</strong> and the agent is confident it has found one, it says
                      so and asks the respondent which answer is right —{' '}
                      <strong>nothing is changed until they reply</strong>. Costs one extra model
                      call per checked turn, and a conflict spends a turn resolving it, so it eats
                      into a tight question budget.
                    </FieldHelp>
                  </Label>
                  <Select
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium">
                        ...
                        <FieldHelp title="Look-back window">
                          How many prior answers to check each new answer against. Must be at least
                          1. A wider window catches a reversal further back in the conversation, but
                          gives the agent more chances to read two compatible answers as a conflict
                          — the default of {DEFAULT_CONTRADICTION_WINDOW_N} is the balance we
                          recommend.
                        </FieldHelp>
                      </Label>
                      <Input
                ...
                <div className="space-y-1.5 sm:max-w-sm">
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Answer fit resolver">
                      ...
                      option the first pass couldn&apos;t place — e.g. &ldquo;Marketing&rdquo; to
                      the &ldquo;Other&rdquo; option, or &ldquo;10 years&rdquo; to the &ldquo;3+
                      ...
                      tries to fill any still-open choice/scale question each turn (more thorough,
                      one extra model call per answered turn). <strong>Off</strong> disables it.
                    </FieldHelp>
                  </Label>
                  <Select
                    value={answerFitMode}
                ...
                <div className="flex items-center gap-2 sm:col-span-2">
                  ...
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Extraction pre-filter">
                      Each turn, narrows the candidate slots the answer extractor reads to the
                      active slot, already-filled slots, same-theme slots, mapped questions, and the
                      most similar to what the respondent just said — cutting per-turn prompt cost
                      on big questionnaires. Spends one embedding call per turn and is fail-soft
                      (any embedding error falls back to the full candidate set).{' '}
                      <strong>Recommended for large surveys</strong> (roughly 50+ data slots / 70+
                      questions); leave off for smaller ones, where sending the full set is cheap
                      and maximises capture accuracy. Off by default.
                    </FieldHelp>
                  </Label>
                </div>
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Definitions in prompts">
                      Folds the definitions relevant to the current question into the prompts that
                      ask it, interpret the reply, refine an answer, and check for contradictions —
                      and the whole set into report generation. This is what stops the interviewer
                      asking about one sense of a word while the extractor reads the answer as
                      another. Only terms that actually appear in the question or the recent
                      conversation are sent, so the cost is a few lines per turn. Default: on.
                    </FieldHelp>
                  </Label>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Respondent hints">
                      ...
                      definition, all are shown as numbered senses. Turn this off if you would
                      rather the definitions guided the agents without steering how respondents
                      answer. Default: on.
                    </FieldHelp>
                  </Label>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Label className="text-sm font-medium">
                    ...
                    <FieldHelp title="Report glossary appendix">
                      Appends the accepted terms and their definitions to the respondent&apos;s
                      report and its PDF, so whoever reads it later knows what the words meant. Off
                      by default because it changes a delivered document. (The blank instrument
                      export always includes the glossary — that one is for you, not the
                      respondent.)
                    </FieldHelp>
                  </Label>
                </div>
                  ...
                  <div className="mb-6 space-y-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      <FieldHelp title="Default placement">
                        ...
                        after the intro and before the conversation — the respondent can&apos;t
                        start until it&apos;s filled in. <strong>Conversation</strong> drops the
                        form and has the interviewer ask for these naturally in the chat. Any field
                        can override this with its own &quot;Collect via&quot; setting — e.g. keep
                        name &amp; email on the form while the rest are gathered in conversation (a
                                  ...
                                  <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <FieldHelp title="Collect via">
                                        ...
                                        follows the version default above. Override it to mix the
                                        two — e.g. keep name &amp; email on the form while the rest
                                        are gathered in conversation. Conversation fields are asked
                                        for naturally in-chat and saved as the respondent provides
                                        them; required ones keep being asked until answered.
                                      </FieldHelp>
                                    </div>
                                    <Segmented
                                      size="sm"
                                  ...
                                  <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <FieldHelp title="Answer checking">
                                        ...
                                        tidies the value (proper-case names, neat organisation
                                        names) and rejects obvious nonsense (&quot;asdf&quot;,
                                        &quot;test@test&quot;). <strong>Both</strong> runs the
                                        format check first, then the AI tidy/flag. The AI pass is
                                        best-effort — if it can&apos;t run, the respondent is never
                                        blocked.
                                      </FieldHelp>
                                    </div>
                                    <Select
                                  ...
                                  <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1.5">
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <FieldHelp title="Storage key">
                                        The internal id this answer is saved under — auto-filled
                                        from the label, lowercase with underscores. You rarely need
                                        to touch it; change it only if another system reads these
                                        answers by a specific key. Avoid renaming it on a live
                                        questionnaire, or previously collected answers won&apos;t
                                        line up.
                                      </FieldHelp>
                                    </div>
                                    <Input
