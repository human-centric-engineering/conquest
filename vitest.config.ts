...
export default defineConfig({
  ...
  test: {
    ...
    // FORK NOTE (ConQuest): a component that renders a live <iframe> (e.g. the
    // intro video embed) should opt into `jsdom` rather than `happy-dom` in its
    // per-file docblock — jsdom silently ignores iframe `src`, whereas happy-dom
    // tries to navigate it (network + noise).
