...
Flag any router object written out by hand instead of built with `createMockRouter()` from `@/tests/types/mocks`. **Run these repo-wide, not over the diff** — a diff-scoped check is blind to every pre-existing violation until someone happens to re-touch those exact lines, so the whole-repo count IS the check. Do not restate the expected count here; run it and read the number, for the same reason `tests/types/mocks.ts` refuses to carry one. That blindness is not hypothetical: the first version of this check was diff-scoped and could not see the 23 files it was written to catch.
