 ...
 *
 * ConQuest allows the two video-embed hosts its intro-video feature can produce.
 * The resolver (`lib/app/questionnaire/intro/video.ts`) only ever builds iframe
 * `src`s on these exact hosts from a validated id — never the admin's raw input
 * — so this allow-list is precisely as broad as the feature itself: no other
 * host can be framed, and a hostile stored value still resolves to `null` (no
 * iframe) before it ever reaches an iframe.
 *
 * These lived in `lib/security/headers.ts` until the Sunrise 0.8.0 sync, when
 * #450 opened this seam; keeping them here means the platform CSP table is no
 * longer fork-edited.
 */
export const appFrameSrc: string[] = [
  'https://www.youtube-nocookie.com',
