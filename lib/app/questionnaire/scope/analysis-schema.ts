 ...
 * and returns the topic set that guidance describes. Nothing it returns is live:
 ...
 * into a topic — a vague eligibility clause, a condition that would need a data slot the
...
const proposedTopicSchema = z.object({
  /** Slug the plan and the blind-spot preference will address this topic by. */
  key: z
    .string()
    .trim()
    .min(1)
...
/**
 * Routing language the analyst recognized but could not turn into a topic — a vague eligibility
 * clause, a reference to something the instrument never defines, a condition that would need a
 * data slot the extractor did not produce. Unlike a topic, `sourceQuote` is
...
export const routingAnalysisSchema = z.object({
   ...
   * (`buildRoutingAnalysisRetryMessage` names topics, gaps, summary and `fromDocument` — not
   ...
   * response: every topic, every gap, gone. Exactly the failure this doc comment says
