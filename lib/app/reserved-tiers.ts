 ...
 */
// FORK FILL (ConQuest) — the two reserved tiers this leaf fork actually fills.
// Declared on the Sunrise 0.10.0 sync (#660): `reserved-fork-tiers.test.ts`
// asserts each reserved directory is empty, which is the promise upstream keeps
// and an unsatisfiable assertion in a fork that was TOLD to fill them.
//
// Deliberately NOT listed: `lib/framework`, `.context/framework` and
// `components/framework`. ConQuest is a leaf fork, not a framework-layer one, so
// those three stay guarded — and the declaration is self-cleaning, since the
// guard fails on a tier declared but left empty.
export const occupiedTiers: readonly string[] = ['components/app', '.context/app'];
