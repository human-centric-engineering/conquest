...
/**
 * Seed the four interviewer-policy evaluation judges (F18.8).
 *
 * Each is a `kind = 'judge'` `AiAgent`, the same shape as the design-evaluation judges
 * (`018-design-evaluation-judges.ts`) and the scope judges (`091`) — editable, deletable, included in
 ...
 * (a structured `runStructuredCompletion` call per dimension) and the `evaluate-policy` capability
...
function judgeInstructions(label: string, summary: string): string {
  return `You are the ${label} in the ConQuest interviewer-policy evaluation panel. You review a \
questionnaire version's INTERVIEWER POLICY — its house rules, questioning arc, and per-question \
ask-as-written dial — before launch and propose concrete edits. ${summary} You score only your own \
...
const unit: SeedUnit = {
  name: 'app-questionnaire/095-policy-evaluation-judges',
  async run({ prisma, logger }) {
    logger.info('⚖️  Seeding 4 interviewer-policy evaluation judge agents...');
    ...
    for (const dimension of POLICY_EVALUATION_DIMENSIONS) {
      ...
      await prisma.aiAgent.upsert({
        ...
        create: {
          ...
          temperature: 0.2,
          maxTokens: 2048,
          // A safety ceiling on each judge's spend, so one can't starve the panel.
          monthlyBudgetUsd: 50,
    ...
    logger.info(`✅ Seeded ${POLICY_EVALUATION_DIMENSIONS.length} policy-evaluation judge agents`);
  },
};
