...
| Generative Authoring     | `.context/orchestration/generative-authoring.md`              | Compose a questionnaire from a plain-English brief (two-phase stream + conversational refine); sibling to document extraction                                                                   |
| Structure Edit Agent     | `.context/admin/questionnaire-edit-agent.md`                  | "Edit with AI" on the Structure editor — instruction → deterministic edit-ops (precise) or whole-doc rewrite; preview then apply                                                                |
| Report Web Search        | `.context/orchestration/report-web-search.md`                 | Optional web-search rounds in report generation (before/after); `web_search` capability (Brave), research agent tool-loop, Research config tab                                                  |
| P15 Follow-ups           | `.context/app/planning/features/f15-followups.md`             | **Everything still open across Experiences** — the erasure gap, why `merged` was deliberately not built, cross-device resume, and the review bugs worth remembering                             |
| Experience Meetings      | `.context/app/questionnaire/experience-meetings.md`           | Facilitated meetings — the three-phase breakout clock, why k-anonymity is structural, what the synthesiser reads, rooms and scribe mode                                                         |
| Experience Reports       | `.context/app/questionnaire/experience-reports.md`            | Per-step cohort reports — why scoping is per step and never per experience, the denormalised session pointer, the third owner key                                                               |
| Experience Continuity    | `.context/app/questionnaire/experience-continuity.md`         | Respondent journey across legs — `/x/<publicRef>` stable address, httpOnly run credential, stitched seam, why the credential is not in the URL                                                  |
| Respondent Designs       | `.context/app/questionnaire/respondent-designs.md`            | The third axis — how the surface is DRAWN (rounded / press / marque); why a design is CSS only, the Tailwind `--radius-*` mechanism, and what the stylesheet guard asserts                      |
| Conditional Topics       | `.context/app/questionnaire/conditional-topics.md`            | Which parts of a questionnaire apply to this respondent — topics, hard rules, the Scope Planner, the Routing Analyst, report/scoring awareness, respondent amendment                            |
| Sectioned Interviews     | `.context/app/questionnaire/sectioned-interviews.md`          | Sectioned interviews (P21): the resolver ladder, the two-sections floor that keeps it inert, the counted turn budget, and why the strip never ships its membership                              |
| Interviewer House Rules  | `.context/app/questionnaire/interviewer-house-rules.md`       | Per-questionnaire behaviour policy (always / never / if-asked) — why it is not tone, the section-order precedence rule, the off-is-silent invariant, the defensive read path                    |
| Question Fidelity        | `.context/app/questionnaire/question-fidelity.md`             | Per-question "ask as written ↔ fill creatively" dial — the five stops, the two no-op layers, why it never overrides Conditional Topics, the fork/export carriage chain                          |
| Turn Progress            | `.context/app/questionnaire/turn-progress.md`                 | The honest wait — the four stage labels between an answer and the reply, the emitter that keeps `runTurn` pure, why the indicator is app-owned not the platform's, and the one-live-region rule |
| Interviewer Strategy     | `.context/app/questionnaire/interviewer-strategy.md`          | The questioning arc — approach (funnel/open/targeted), the pace dial and its four-number profile, opening framings, the opening-questions assistant, and what the arc actually did              |
| Interviewer Policy Panel | `.context/app/questionnaire/interviewer-policy-evaluation.md` | The third judge panel — house rules × arc × fidelity; why the scope panel's "no collision" argument is false here, the borrowed Zod, per-dimension prompt sections                              |
| AI Run Provenance        | `.context/app/questionnaire/ai-run-provenance.md`             | Which AI runs are preserved vs ephemeral and why; `AppAiRun`, `recordAiRun`, superseded extraction changes, app cost attribution, app retention                                                 |
| Brand Import             | `.context/app/questionnaire/brand-import.md`                  | Propose a client's theme from a screenshot of their site — the measure-then-assign split, why the model can never invent a hex, the four-outcome failure contract                               |
| Brand Contrast           | `.context/app/questionnaire/brand-contrast.md`                | Optimise a client's theme for legibility — the tint/shade ramp (and why not HSL), the two WCAG thresholds, why the model picks an index rather than a colour                                    |
| Questionnaire Pack       | `.context/app/questionnaire/questionnaire-pack.md`            | Branded PDF/CSV/Markdown export (title, questions, data slots, definitions, experience setup) with per-section include toggles; distinct from the brand-free instrument export                  |
