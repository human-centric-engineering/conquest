 ...
 *   it also dispatches the offer-composer capability to phrase the offer (the "agent
 *   contract"); fail-soft.
 *
 *   The deterministic assessment is free; the paid LLM phrasing takes a per-admin
...
const handleCompletionStatus = withAdminAuth<{ id: string; vid: string }>(
  async (request, session, { params }) => {
    ...
    // Compose the (paid) offer phrasing only when the gate says we may offer — the
