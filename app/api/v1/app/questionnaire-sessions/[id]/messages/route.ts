 ...
 *
 * Gate order: live-sessions flag (404 before auth) → withAuth → load session → ownership
 * (respondent must own it) → status must be `active` → per-turn sub-cap → body validation.
 * Per-step sub-flag gating: a disabled sub-feature is skipped gracefully (the turn still
 * runs). Capability failures are fail-soft (a `warning` frame, never a 5xx once streaming).
 */
...
const handleMessage = withAuth<{ id: string }>(async (request, session, { params }) => {
  ...
  // Ownership: the authenticated respondent must own the session. The no-login anonymous
  // path (respondentUserId null) is a separate surface (PR6); here it's a 403.
  if (loaded.session.respondentUserId !== userId) {
    return errorResponse('You do not have access to this session', {
      code: 'FORBIDDEN',
      status: 403,
    });
  }
  ...
  const limit = turnLimiter.check(userId);
