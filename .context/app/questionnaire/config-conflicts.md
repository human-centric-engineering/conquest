...
The house-rules and opening checks describe settings that live on this tab. The `conditional-topics-*`
three do not: Conditional Topics is authored on the **Topics** tab and has its own server-side coherence
checker (`validateConditionalTopics`), which answers "is this routing setup well-formed". These answer a
...
All three anchor to `interviewer-strategy`, not to Conditional Topics: that is the setting the admin
would change, and it is the one on this tab. They read `config.conditionalTopics`, which `ConfigEditor`
