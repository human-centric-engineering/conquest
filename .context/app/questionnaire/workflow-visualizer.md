...
`scope` selects **which settings blob** `key` indexes into: omitted (or `'questionnaire'`) means `QuestionnaireConfigShape`; `'experience'` means `ExperienceSettingsShape` on `AppExperience`, a different blob on a different Settings tab. It exists for the three Experience diagrams, whose steps are governed by experience-level settings (`insightMinSupport`, `summariseCarryOver`, `breakoutGraceSeconds`, …). The integrity test resolves each key against the matching defaults object, so **both** scopes stay pinned — dropping the field instead would have cost the operator the "where do I change this?" answer that is the whole point of `_meta.settings`.
...
**The three Experience diagrams** (`experience-switcher`, `experience-meeting`, `experience-run-lifecycle`) document `lib/app/questionnaire/experiences/**`, which is hand-written TypeScript orchestration like every other diagram here — **not** rows in the platform's editable workflow engine. They must never be added to `/admin/orchestration/workflows`: a non-executable entry inside an editable surface invites someone to press Run. Their three agents (`app-experience-router`, `app-experience-handoff`, `app-meeting-synthesiser`) build prompts in code from run-time material rather than from a catalogued template, so they are on the integrity test's `UNCATALOGUED_AGENT_SLUGS` allowlist and their nodes correctly show no Prompt tab. Their slugs live in `experiences/constants.ts` rather than the shared `questionnaire/constants.ts`, so the test unions that module in separately. Applicability is always `applies`: an experience composes whole questionnaires and is not scoped to a single version, so the lens has nothing to gate on.
