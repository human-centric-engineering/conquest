...
**Node info panel** (on click) shows, top to bottom: the step's **Role** (its `WorkflowStep.description`), the **Settings that affect this step** (`_meta.settings` — each a `{ key, label, effect, scope? }` where `key` is a dotted path into the settings blob named by `scope`, so an operator can find it on the Settings tab), then the **Agent / Prompt / Knowledge / Tools** tabs. The per-workflow **Purpose** (the diagram `description`) shows above the canvas.

`scope` selects **which settings blob** `key` indexes into: omitted (or `'questionnaire'`) means `QuestionnaireConfigShape`; `'experience'` means `ExperienceSettingsShape` on `AppExperience`, a different blob on a different Settings tab. It exists for the Experience diagrams, whose steps are governed by experience-level settings (`insightMinSupport`, `summariseCarryOver`, `breakoutGraceSeconds`, …). The integrity test resolves each key against the matching defaults object, so **both** scopes stay pinned — dropping the field instead would have cost the operator the "where do I change this?" answer that is the whole point of `_meta.settings`.
...
| `experience-switcher`      | entry leg → carry-over → budget gate → rules (first-match) → selector → confidence → fallback → handoff → follow-up leg → conclude | `experiences/routing/select.ts`          |
| `experience-meeting`       | meeting opens → join → breakout opens → rooms → running → grace → closed → material → synthesise → k-anonymity → console           | `experiences/meeting/synthesise.ts`      |
| `experience-run-lifecycle` | stable address → run credential → leg A → seam (linked \| stitched) → leg B → conclude → run report                                | `experiences/run/types.ts`               |
...
**Deep links.** The page accepts `?workflow=<slug>` and passes it to the explorer as `initialSlug`; an unknown slug falls back to the first workflow rather than rendering an empty canvas. The experience workspace's **How it works** tab links in this way, so the two surfaces show the same curated artefact instead of two descriptions that drift.

**The four Experience diagrams** (`experience-switcher`, `experience-meeting`, `experience-run-lifecycle`, `experience-synthesis`) document `lib/app/questionnaire/experiences/**`, which is hand-written TypeScript orchestration like every other diagram here — **not** rows in the platform's editable workflow engine. They must never be added to `/admin/orchestration/workflows`: a non-executable entry inside an editable surface invites someone to press Run. Their four agents (`app-experience-router`, `app-experience-handoff`, `app-meeting-synthesiser`, `app-experience-synthesiser`) build prompts in code from run-time material rather than from a catalogued template, so they are on the integrity test's `UNCATALOGUED_AGENT_SLUGS` allowlist and their nodes correctly show no Prompt tab. Their slugs live in `experiences/constants.ts` rather than the shared `questionnaire/constants.ts`, so the test unions that module in separately. Applicability is always `applies`: an experience composes whole questionnaires and is not scoped to a single version, so the lens has nothing to gate on.
