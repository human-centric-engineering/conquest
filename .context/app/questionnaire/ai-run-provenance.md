...
| Kind                | Surface                          | Why                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `extraction_verify` | ingest + re-ingest stream routes | Questions flagged `suspect` but not repaired used to persist looking exactly as clean as confirmed ones. `repairOutcome` distinguishes `repaired` / `repair_failed` / `skipped_systemic` / `verifier_unavailable` — `repair_failed` is set when the repair pass returned zero repairs (unseeded agent, failed dispatch, or throw), so a fail-soft repair can't be filed as a successful one.                                                                             |
...
| `scope_candidacy`   | ingest + re-ingest routes        | The cheap "does this document describe routing" triage read. Its verdict is also cached on the version, so the run row is the history rather than the state.                                                                                                                                                                                                                                                                                                             |
| `evaluation_steer`  | evaluation batch apply           | The one place in the evaluation flow where a **model's own words reach the questionnaire**: a judge only proposes and an admin adjudicates, but here the admin accepted a suggestion and a model then rewrote it to follow their instruction. `detail` carries the instruction, the note and anything `unhonoured`. Kept on failure too — "the model returned a different kind of change and we refused it" is the answer to a reviewer's later "why didn't this apply". |
| `routing_analysis`  | analyse SSE + streaming ingest   | **Load-bearing, not just provenance.** Its `status` is what `resolveAutoTriggerPending` reads: a `succeeded` run stops the Topics tab re-proposing forever (surviving a discard, which deletes the draft), and `failed` runs are counted and tolerated up to two before the automation gives up. `detail.trigger` says whether an admin asked or an upload did.                                                                                                          |
