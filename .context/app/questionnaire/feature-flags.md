...
The questionnaire product dark-launches behind **one master flag and twelve sub-flags**. This
...
| 13  | `APP_QUESTIONNAIRES_FRICTIONLESS_INVITES_ENABLED`    | `isFrictionlessInvitesEnabled`                            | master **+ live-sessions** | per-invitee token that boots a no-login session (answer without registering; optional account for resume)                                                                          | invitations fall back to the account-registration accept flow                                                                                                                                                                      |
| 14  | `APP_QUESTIONNAIRES_INVITE_IMPORT_ENABLED`           | `isInvitationImportEnabled`                               | master                     | the import wizard's CSV/PDF/image methods + the paid AI people-extraction capability                                                                                               | the admin adds invitees by typing them directly (no CSV/PDF/image import, no AI extraction)                                                                                                                                        |
| 15  | `APP_QUESTIONNAIRES_TONE_ENABLED`                    | `isToneEnabled`                                           | master **+ live-sessions** | F-tone interviewer tone & persona — fold the per-version tone sliders + persona into the live phraser's prompt                                                                     | the interviewer keeps its **default voice** even when a version enables tone dimensions (no 404 — it gates a behaviour inside the messages route). See [interviewer-tone.md](./interviewer-tone.md)                                |
| 16  | `APP_QUESTIONNAIRES_COHORTS_ENABLED`                 | `isCohortsEnabled` / `ensureCohortsEnabled`               | master                     | Cohorts & Rounds — admin cohort/round CRUD + the demo-client Cohorts/Rounds tabs, AND the respondent round access guard (window + active membership)                               | the cohort/round routes **404** and the demo-client tabs hide; the access guard is **inert** (no session carries a `roundId`, so it never fires). See [cohorts.md](./cohorts.md)                                                   |
| 17  | `APP_QUESTIONNAIRES_INGEST_VERIFY_REPAIR_ENABLED`    | `isIngestVerifyRepairEnabled`                             | master                     | the ingest verify + repair pass — a critic that flags unfaithful extracted questions + a scales/matrix repair specialist that re-extracts the flagged ones (streaming ingest only) | ingestion is the **single-extractor pass** (no verify, no repair — no 404; it gates a behaviour inside the streaming ingest route). See [ingestion.md](./ingestion.md)                                                             |
