...
| [`chat-text-size.md`](./chat-text-size.md)                   | Respondent-owned chat text size — a lifecycle-strip stepper scaling the transcript via one inherited `--cq-chat-scale` custom property; global (not per-session) localStorage, clamping ladder, why it is deliberately not an admin config knob               |
...
| [`maintenance-cron.md`](./maintenance-cron.md)               | Why the maintenance cron runs hourly, not per-minute — the Neon autosuspend cost, the audit showing nine of twelve tick tasks are no-ops for ConQuest, and the four conditions that would invalidate hourly (UG-13 / sunrise#442)                             |
