...
| **Down-propagation** (fill the form from a fill)        | `lib/app/questionnaire/capabilities/opportunistic-fill.ts` (`selectOpportunisticTargets`, `buildFreeTextOpportunisticIntents`, `capOpportunisticConfidence`) | A confident data-slot fill seeds its **unanswered** mapped questions, provenance `inferred`, never overwriting an already-answered question. **Two ceilings, deliberately different:** free-text seeds copy the paraphrase down at a flat `OPPORTUNISTIC_CONFIDENCE_CAP` (0.45 — Tentative, no per-question fit judgment to trust). Choice/likert go through the answer-fit resolver, whose confidence _is_ a judgment of mapping clarity, so we **preserve** it up to `OPPORTUNISTIC_TYPED_CONFIDENCE_CAP` (0.75) instead of flattening — a clearly-pinned likert reads "Fairly sure", not "Tentative", even when derived tangentially and stated briefly. The ceiling stays below "Confident" (0.85), which corroboration earns via accrual. A loose fit keeps its own lower score. Numeric/boolean/date are out of scope. |
