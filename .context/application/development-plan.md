...
- Doc namespace at `.context/app/questionnaire/` — `README.md`, `overview.md`, `schema.md`, `development.md`, plus an empty forward-looking `upstream-gaps.md`.
...
- **2026-05-31 — CI assumes a public repo; carried overrides for a private fork.** `pending-upstream`. Sunrise's CI workflows assume the repo is public, which breaks on a private fork like ConQuest. Three overrides carried in `.github/workflows/`:
  ...
  - `Dockerfile` — added `ENV NODE_OPTIONS=--max-old-space-size=4096` to the `builder` stage (build-time only; the `runner` stage does not inherit it, so production runtime memory is unchanged). The `validate` heap bump does not reach inside the container, so the `docker` job's `next build` OOM'd separately on the 7GB runner.
