...
7. **`done`** — terminal. Carries `tokenUsage` (sum for the final turn), `costUsd` (final turn cost only), `provider` (the resolved provider slug, useful when fallback activated), `model` (the model id used), and `finishReason` (optional) — why the provider stopped on the **final** turn of the tool loop.

   `finishReason: 'length'` means the assistant text is a **fragment cut off at the token cap**, not a complete answer. It exists because a consumer that parses the text otherwise cannot tell "the model produced the wrong shape" from "the model was interrupted mid-shape", and reports the second as the first — which is exactly what the evaluation judge did, recording `score: null, reasoning: "not valid JSON"` for a judge that had merely run out of tokens (#594). Optional and additive: absent when the provider reports nothing. `drainStreamChat` surfaces it on `DrainResult` for non-SSE callers.
