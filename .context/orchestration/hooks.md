...
| `agent.updated`                 | `app/api/v1/admin/orchestration/agents/[id]/route.ts` (also dual-dispatched as `agent_updated` to webhook subscriptions; payload: `{ agentId, agentSlug, actorUserId, changes: { field: { from, to } } }` with values >500 chars truncated; suppressed when the PATCH produced no actual change) |
