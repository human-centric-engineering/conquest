...
| `anthropic`         | Claude 4 Opus (any) and Claude Sonnet 4.5+ (per `supportsReasoningEffort()`) | `minimal` → no thinking. `low` / `medium` / `high` → `thinking: { type: 'enabled', budget_tokens: 1024 / 4096 / 16384 }`. Budget is clamped to `min(requested, max_tokens − 1024)` so visible output always has ≥ 1024 tokens of room. **Anthropic API requires `budget_tokens ≥ 1024`**, so the provider drops thinking entirely when `max_tokens < 2048` (no way to fit the minimum budget alongside the visible-output floor). Temperature is omitted when thinking is enabled (Anthropic locks it to default). |
