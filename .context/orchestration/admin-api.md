...
| `/api/v1/admin/orchestration/executions/:id/status`                  | GET                | Lightweight status read (no trace, polling-friendly)                                                                                                                                                                                                               |
...
| `/api/v1/admin/orchestration/maintenance/tick`                       | POST               | Returns 202 once schedules are claimed; eight other maintenance tasks run in the background, each on its own minimum interval. Returns 200 `{skipped: true, reason}` when the idle gate is armed or a previous tick is still running; `?force=1` sweeps regardless |
