...
### R012 — 2026-08-26 · **PARTIAL (doc 07 ×1 — the restraint fixes on the document that names them)**

| Field                      | Value                                                                                 |
| -------------------------- | ------------------------------------------------------------------------------------- |
| Commit                     | `f99a07c1e` on `fix/ingest-fidelity-phase-2` (clean tree — `.design/` untracked only) |
| Ran by                     | Claude (agent), driven by John                                                        |
| How                        | streaming ingest via `POST /questionnaires/stream` · doc 07 ×1                        |
| Extractor model            | `openai/gpt-5.4`                                                                      |
| Critic model               | `openai/gpt-5.4`                                                                      |
| Candidacy model            | `openai/gpt-5.4-mini`                                                                 |
| Analyst model              | `openai/gpt-5.4`                                                                      |
| Conditional topics enabled | no (fresh version from the ingest)                                                    |
| Total cost / wall time     | $0.041 recorded on `AppAiRun` · 79s                                                   |
| Environment                | local dev DB, dev server on :3020 · version `cmtagtuvm036qe55ncqeakwsi`               |

Companion to R011: the same post-R010 fixes, on the document the whole restraint band was built
around. Same build, one run.

| Doc | Difficulty | P   | C   | T   | G   | R   | /10 | Failed at | Critical | Note                                                              |
| --- | ---------- | --- | --- | --- | --- | --- | --- | --------- | -------- | ----------------------------------------------------------------- |
| 07  | 5          | 2   | 1   | 0   | 2   | 2   | 7   | analyst   | **YES**  | All five triggered blocks still coerced — now declared, five gaps |

**Doc 07: 7/10, up from 6/10 at R010. The critical failure stands, exactly as the ledger said it would.**
**Verdict: not shippable — and the number moving up while the verdict does not is the point.**

#### Five blocks, five gaps, one per block — the silence half of T07 is fixed

Every triggered block now carries its own gap, quoting its own trigger sentence and naming the
consequence in plain terms:

| Block             | What the gap says will actually happen                                                                                                          |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Domestic abuse    | _"must be added immediately whenever the disclosure happens… only be included if that condition is already apparent by the end of the opening"_ |
| Care leaver       | _"the platform cannot add a topic after the opening has finished"_                                                                              |
| Health/disability | _"explicitly meant to be triggered even by a late or passing disclosure during another answer"_                                                 |
| Immigration       | _"the platform cannot interrupt and add a new topic later"_                                                                                     |
| Rent arrears      | _"specifically allows the block to be added after another block has already been asked"_                                                        |

The summary says it too, unprompted: _"the triggered blocks were proposed as the closest fit but also
reported as gaps because the platform cannot add topics mid-interview."_ At R010, both runs of this
document filed **one** gap about trigger timing in general and converted all five blocks without
further comment.

**And the critical failure is unchanged, deliberately.** The corpus test is _"any of the five
triggered blocks rendered as an opening-time criterion"_, and all five still are — every one carries
a criterion built from its own trigger sentence, so a disclosure in minute 40 still adds nothing.
The post-R010 note called this in advance: the coercion cannot be removed by a prompt, only the
silence, and only the silence was. `T = 0` for the same five membership errors as R010.

`R = 2` is the one score that is a judgement rather than a reading, and it is the same judgement
made for doc 10 in R011: nothing the product cannot express is left undeclared here, so the rubric's
`0` condition ("coercion of a trigger") is read as _silent_ coercion, which the ledger's own post-fix
text says is what was fixed. Read literally instead, this is a `0` and the score is 5/10. **The
ambiguity is real and it is T15's** — recorded rather than resolved, because a rubric edit made to
justify a score is the failure mode this file exists to avoid.

#### The one thing that went backwards: candidacy stopped quoting

`C = 2 → 1`. The verdict is right and confident (`isCandidate: true`, `0.99`) and the three signals
are the right three — everyone-vs-conditional, standing conditions, mid-interview triggers. But
**two of the three carry no `sourceQuote` at all**:

> _"The framework explicitly says some blocks are asked of everyone while others depend on conditions
> and triggers."_ · _"It states that standing conditions determine which blocks apply for a
> respondent."_

Both assert what the document says, and both are quotable — the "How this framework works" preamble
states each in a sentence. The rubric scores that as thin (`1` — "correct verdict but no quotes"),
and unlike doc 06's unquoted signals there is no absence being described here. `candidacy-prompt.ts`
does license the omission — _"When a signal is inferred rather than quoted, OMIT sourceQuote
entirely"_ — which is what makes this worth watching rather than obviously wrong. See **T16**.

#### Grounding was perfect, and it is the first restraint-band run that was

`G = 2`. All eleven topic quotes and all five gap quotes are **contiguous, byte-exact spans** of
`extractedText` — no assembled quotes, no elisions, no re-rendered emphasis. R010 found assembled
quotes on both hard documents and R011 found seven on doc 10; this run has none. Fourth data point
for T10 being variance-prone rather than universal.

#### Extraction was clean

`P = 2`. All 11 question-bearing sections, all 36 questions, in source order, none invented, none
dropped, `boolean` inferred on the two yes/no questions and `free_text` on the rest. Two
`prune_section` changes removed the title block and the interviewer's "How this framework works"
preamble — correct, that is routing guidance rather than questions, and the analyst still read it
from the source text.

Five `rewrite_prompt` changes, four cosmetic (_"How long have you been there?"_ → _"…been living
there?"_) and one that adds meaning: _"What status do you currently hold?"_ → _"What immigration or
residence status do you currently hold?"_. Defensible in a section titled immigration status, but it
is the extractor writing words the author did not — T05 again, now declared rather than silent.

#### Changed since last run

- **Nothing in the build.** R011 and R012 both ran on `f99a07c1e`, so doc 07 and doc 10 are
  comparable to each other and both are comparable to R010's `f4ab51f48` for everything except the
  post-R010 fixes themselves.
- **The run's questionnaire was left in place** (`cmtagtuve036pe55njujzw2q8`, titled
  `R012 — 07 housing needs`) and the draft was **not** accepted.

---

### R011 — 2026-08-26 · **PARTIAL (doc 10 ×1 — the first score of the post-R010 restraint fixes)**

| Field                      | Value                                                                                 |
| -------------------------- | ------------------------------------------------------------------------------------- |
| Commit                     | `f99a07c1e` on `fix/ingest-fidelity-phase-2` (clean tree — `.design/` untracked only) |
| Ran by                     | Claude (agent), driven by John                                                        |
| How                        | streaming ingest via `POST /questionnaires/stream` · doc 10 ×1                        |
| Extractor model            | `openai/gpt-5.4`                                                                      |
| Critic model               | `openai/gpt-5.4`                                                                      |
| Candidacy model            | `openai/gpt-5.4-mini`                                                                 |
| Analyst model              | `openai/gpt-5.4`                                                                      |
| Conditional topics enabled | no (fresh version from the ingest)                                                    |
| Total cost / wall time     | $0.149 · 85s                                                                          |
| Environment                | local dev DB, dev server on :3020 · version `cmtagb3a20333e55n4mdpc8sm`               |

**One document, one run, and deliberately so.** R010 scored doc 10 at 4/10 with a critical failure;
the post-R010 gaps-rubric fixes then measured 1 → 6 gaps on the live pipeline but were never scored.
This is that score, on the committed build.

| Doc | Difficulty | P   | C   | T   | G   | R   | /10 | Failed at | Critical | Note                                                                 |
| --- | ---------- | --- | --- | --- | --- | --- | --- | --------- | -------- | -------------------------------------------------------------------- |
| 10  | 5          | 1   | 2   | 0   | 1   | 1   | 5   | analyst   | **none** | Both stop conditions declared as terminations; T07 coercion persists |

**Doc 10: 5/10, up from 4/10 at R010.** No corpus or band score — one document.
**Verdict: the named critical failure is cleared; the document is not.**

#### The stop condition is no longer discarded, and that is the whole result

The corpus' critical test for doc 10 is _"the terminating screener absent from `gaps[]` — a stop
condition discarded"_. It is now present twice, in termination language, unprompted:

> _"This is a termination instruction, not just a routing condition. The questionnaire can ask about
> tenure, but it cannot end the interview or switch the respondent into the New Partner Check-in; the
> review would continue regardless."_

> _"This is a termination and referral instruction that cannot be formalized as a topic or hard rule
> here. There is also no question in this instrument that captures formal-dispute status, so the stop
> consequence cannot be enacted."_

The second gap goes further than the rubric asks: it notices that the instrument contains no question
capturing dispute status, so the condition could not be evaluated even if it could be acted on. Six
gaps in total — both stop conditions plus one per escalation trigger, each quoting its own trigger
sentence and stating what will actually happen ("included only if the trigger is already apparent by
the end of the opening, not if it first arises later"). At R010 the same document filed one gap on
run 1 and three on run 2, and never once said the screener ends the review.

`R` moves 0 → 1 rather than 0 → 2, for one reason: **mutual exclusivity is still neither expressed
nor declared.** _"a partner takes one of these, not several"_ is quoted inside all three role topics'
`sourceQuote`s and then goes unmentioned — the schema cannot enforce one-of-three, so it belongs in
`gaps[]`, and R010 run 2 did put it there. See **T14**.

#### T and G did not move, and T is where the ledger now argues with the product

`T = 0` is scored against the README's unchanged ground truth: the four escalation blocks _"should
land in `gaps[]`"_ and all four are proposed as conditional topics — four membership errors. That is
the same score, for the same reason, as R010.

**But the product now does this on purpose.** The post-R010 gaps rubric says to propose the topic
_and_ gap it, because "a block belonging to no topic is worse — `validate.ts` makes its questions
unaskable". So the analyst is being marked down for following an instruction that was added because
the alternative is worse. Recorded, not resolved: the ground truth was deliberately left alone so the
number stays ugly until the product can express mid-interview triggers, and moving it to bank a point
is exactly what the ledger exists to prevent. **Raised as a scoring question for a human**, not
actioned — see **T15**.

`G = 1`: no fabricated quotes. Every quoted part is byte-exact against `extractedText`, verified with
markdown normalisation and split on `…` / `;`. Seven quotes are **assembled** (T10) — four topics
stitch a parent heading to a sub-heading (`"## Standard sections — every partner" + "### People"`),
and three gaps join the trigger preamble to a non-adjacent bullet. Fifth document for T10.

`C = 2`: candidacy correct at `0.98` with three signals, one per mechanism (screening, role routing,
escalation triggers), all three byte-exact.

#### What broke — `P = 1`, and it is the same misplacement R010 saw

All 36 source questions extracted, none invented, none dropped, types sane (`numeric` on the three
counting questions). The structure is wrong in one place: the extractor **added an Eligibility
section** — declared honestly as an `add_section` change, _"to preserve the document's initial
screening logic"_ — and moved **"How many sites do you operate today?"** into it, leaving Opening
with four of its five questions. The document's eligibility block contains no numbered questions at
all, and the fact its rules turn on is tenure, not site count: `network_tenure` stayed in Opening
while the sites question moved. R010 run 2 did the same thing. 13 sections against the source's 12
question-bearing ones follows from the same change.

Four `rewrite_prompt` changes, all cosmetic (`"the standards"` → `"that standards"`, `"What
standard"` → `"Which standard"`, an inserted comma, `"the thinking"` → `"this thinking"`) — T05
again, and all four now declared rather than silent, which is what `bbfbd28d1` was for. `changeCount`
52 = 36 `infer_type` + 8 `augment_question` + 4 `rewrite_prompt` + 1 each `add_section`,
`prune_section`, `infer_goal`, `infer_audience`.

#### Changed since last run

- **Build moved: `f4ab51f48` → `f99a07c1e`** — the post-R010 fixes (T13's key-length cap, T12, and
  the three gaps-rubric exceptions: timing, termination, contradiction-within-one-document).
- **T13 did not fire.** Longest minted question key on this version is 46 characters, well inside the
  64 cap that failed doc 08 run 2.
- **The run's questionnaire was left in place** (`cmtagb39v0332e55n31272yaj`, titled
  `R011 — 10 franchise operations`) rather than archived, so the proposal can be read on the Topics
  tab. The draft was **not** accepted.
...
## Post-R010 fixes — what changed, and what deliberately did not

Applied after R010 banked the single-build baseline, so the 84.0% above remains the last measurement
of the unfixed pipeline and the next full run measures these.

**T13 and T12 — plain bugs, fully fixed.** See their rows below. Neither touches a scored dimension.

**T07 — half fixed, and the half that is left is still yours.**

The critical failure is _silent_ coercion, and it has two halves. The coercion cannot be removed by a
prompt: scope is settled once, when the opening completes, and until that changes a trigger that
fires in minute 40 cannot add a block. **The silence can be, and was.**

Three exceptions were carved into the gaps rubric, because the R005 fix — right for the tautology it
was aimed at — also told the analyst not to declare a trigger it had just converted:

1. **Timing.** A block the document adds "at any stage" / "whenever they surface" / "even in
   passing" is still a gap even though you expressed it, because the criteria you wrote do not mean
   what the document said. **One gap per block**, quoting that block's own trigger sentence and
   saying what will actually happen. Still propose the topic — a block belonging to no topic is
   worse, since `validate.ts` makes its questions unaskable.
2. **Termination.** An instruction that ENDS the interview ("stop the review", "end the conversation
   here") is inexpressible: `SCOPE_RULE_ACTIONS` is `include | exclude`, nothing halts an interview,
   and excluding every topic is a different thing. Turning the screener's checks into opening
   questions "captures the fact and discards the consequence".
3. **Contradiction.** "Do not pick a side quietly" existed but fired **only when a supplementary
   document was attached** — so doc 08, which contradicts itself inside one file, never triggered
   it. Now applies within a single document, quoting both sides.

**Measured on the live pipeline, same build, before → after:**

| Doc | Gaps before | Gaps after | What is now declared                                             |
| --- | ----------- | ---------- | ---------------------------------------------------------------- |
| 07  | 1           | **5**      | one per triggered block, each quoting its own trigger sentence   |
| 08  | 1           | **3**      | three contradictions, each quoting BOTH the table and the note   |
| 10  | 1           | **6**      | **both stop conditions** as terminations, plus all four triggers |

Doc 10's two new gaps say it outright: _"This is a termination instruction, not just a scoping
condition… the platform cannot end the interview or route the respondent into a different review, so
this review will continue."_

**The corpus ground truth was NOT changed, and doc 07 should still fail the next run.** Its
critical-failure test is "any of the five triggered blocks rendered as an opening-time criterion",
and all five still are. Moving that goalpost to bank a higher score would make the ledger worthless —
the number exists to stay ugly until the product can do what the document says. What should improve
is `R` on docs 08 and 10, where the rubric scores "a contradiction silently resolved" and "a stop
condition dropped", and both are now declared rather than dropped.

**Still open, and still a product decision:** whether Conditional Topics gains a mid-interview
re-scope mechanism (with the report-reproducibility and cohort-comparability consequences the README
names), or an explicit refusal path that keeps a triggered block visible to the admin as
not-routable. Nothing above prejudges it.
...
> **`ext-A` is an off-corpus run, and it is not a corpus score.** One private client instrument,
> ingested twice on `1b52bd5ac`, to put the chain in front of a real document rather than one written
> for this repository. It is deliberately absent from the run log and contributes to no corpus number —
> a corpus score is still the ten documents and nothing else. It is recorded here **by shape only**,
> because the file is confidential: a five-tab `.xlsx` workbook, 30,678 characters flattened, 16
> sections, 69 questions, and twelve signal→section routing rules against a stated cap of three, with
> its routing guidance on dedicated tabs at the back. Nothing it raised below depends on knowing more
> than that.

| ID  | Raised                 | Docs           | Status                                            | What was seen                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Candidate tweak                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --- | ---------------------- | -------------- | ------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| T18 | ext-A                  | ext-A          | open                                              | The instrument's guardrail tab states a cap on probing during the opening (one, for the whole opening, with the reason spelled out) and a whole-interview time budget, from which a further tab derives the seconds left for routed sections. **Both are settings this product already implements** — `limitOpeningProbes` / `maxOpeningProbes` in `scope/types.ts`, and `PlanBudget` on the planner — and **neither has a field in `routingAnalysisSchema`**, so the analyst cannot propose either. It reported neither, on both runs. This is not a failure to decline: the prompt's three gap exceptions (timing, termination, contradiction) do not cover a setting, and `analysis-prompt.ts` closes by telling it not to gap anything `fallbackTopicKeys` or `checkTopicPreference` can express — which reads, reasonably, as "settings are not gap material". The cost is an admin who must read the guardrail tab themselves and set both by hand, on a proposal that otherwise looks complete.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | **Extend the schema; do not touch the prompt.** Add the opening-probe cap and the plan budget as optional fields on `routingAnalysisSchema`, on the same omitted-means-leave-alone contract as `maxConditionalTopics`. This is the F17.23 move exactly, and its own docblock in `_lib/topic-draft.ts` makes the argument: _"before this, a document that named a safe default … came back as an unformalizable `gap`, which was the proposal admitting defeat about a setting the platform had implemented all along."_ Same sentence, two more fields. No prompt judgement is involved, so the parking rule does not really bind here — this is the "plain bug" end of the table, a capability gap rather than a calibration call. Until it is done, the honest behaviour is to gap them rather than stay silent.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| T17 | ext-A                  | ext-A          | open                                              | The instrument ranks each of its twelve signal→section rules **High / Medium / Fallback**, against a stated cap of three conditional topics — so for any respondent tripping four or more rules, which is the ordinary case and not the edge, the ranking is what the author intended to decide the interview. The proposal was otherwise complete and correct (12 conditional topics, every rule fanned out to the right one, cap and fallback both read off the document), and **the ranking appears nowhere**: not in any topic's `criteria`, not in `gaps`, not in the summary. Identical on both runs. Verified that it reaches nothing at runtime: `guardrails.ts` seats _"the model's picks, in its own order, up to what remains of the limit"_, and the Scope Planner orders from the topics' `criteria`, which carry no priority — while the `(high priority)` convention in `criteria-format.ts` is parsed **only** by `routing-map-detail.tsx`, for display. Within instructions throughout: none of the prompt's three gap exceptions covers arbitration, and it is told plainly that zero gaps is the common and correct answer. **The risk is the README's coercion risk arriving by omission** — an admin reads a complete-looking proposal with nothing to tell them the author's ranking went nowhere.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Two separable pieces; take them in order. **(1) A fourth gap exception — ARBITRATION:** the document ranks, or forbids combinations among, the conditional topics just proposed. Cheap, prompt-only, and it makes the silence visible without pretending to fix it. T14 is the same family and should fold in. **(2) Whether a stated priority should actually reach the planner is a product decision, not a prompt tweak** — it means a per-topic rank the planner is obliged to respect, which is a new input to a component that currently decides by fit to what the respondent said. Those two orderings usually agree, which is what makes the disagreement easy to miss. **Do not take the middle path** of having the analyst write `(high priority)` into the criteria text: it parses, but only for display, so it would show the admin a ranking that still changes nothing — worse than a gap, because it looks handled. **Hold (1) for a second document.** No corpus document ranks: 05 and 09 state breadth CAPS ("no more than four", "prioritise no more than three"), which is a different instruction, so nothing in the ten can confirm this. It waits for a doc 11 written for it, or the next real instrument. Needs John for (2).                                                                                                                      |
| T16 | R012                   | 07             | open                                              | Doc 07's candidacy filed three signals and **two carried no `sourceQuote`** — _"The framework explicitly says some blocks are asked of everyone while others depend on conditions and triggers"_ and _"It states that standing conditions determine which blocks apply for a respondent"_. Both assert what the document says, and both are quotable from the "How this framework works" preamble. `candidacy-prompt.ts` licenses the omission for a signal that is _inferred_ rather than quoted, so this is within instructions; the question is whether the licence is being used where a quotable sentence exists. Cost `C` a point (2 → 1 against R010) on a document whose candidacy was previously fully quoted. Distinct from doc 06's unquoted signals, which describe an **absence** and cannot be quoted by construction.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Watch for a second occurrence before touching the prompt. If it recurs, the candidate is a narrowing of the licence — omit `sourceQuote` only when the signal genuinely cannot be quoted (an absence, a structural inference) rather than whenever the model chose to paraphrase.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| T15 | R011, R012             | 07, 08, 10     | open                                              | **R012 put a number on the ambiguity: doc 07 scores 7/10 or 5/10 depending on which reading of `R` is taken**, with everything else about the run identical. **A scoring question, not a pipeline fault.** The post-R010 gaps rubric tells the analyst to propose a mid-interview-triggered block as a conditional topic **and** file a gap for it, because a block belonging to no topic makes its questions unaskable. The README ground truth says those blocks _"should land in `gaps[]`"_, so `T` scores each one as a membership error — doc 10 scores `T = 0` for doing exactly what it was told. The ground truth was deliberately not moved (R011). Needs a human decision: either `T` stops counting a proposed-and-gapped block as an error, or the ground truth is restated to say "propose it, and gap it". Do not resolve it by editing the number.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| T14 | R010, R011, ext-A      | 10, ext-A      | **confirmed — promoted by a second document**     | Doc 10 says _"a partner takes one of these, not several"_ — a mutual-exclusivity constraint the schema cannot express, which the README puts in `gaps[]`. R010 run 1 missed it, run 2 gapped it, R011 quoted the sentence inside all three role topics' `sourceQuote`s and never mentioned it. Two of three runs silent. It is the only inexpressible thing on this document that is still undeclared now the stop conditions are gapped, and it is what holds `R` at 1. **Promoted by `ext-A`, which is the same constraint in a different shape.** That instrument's routing table marks one of its twelve rules mutually exclusive with another and states which to take when both fire — so it carries an exclusivity AND a tie-break, and neither is expressible. Neither was gapped, on either run. Doc 10's role sections are therefore not a special case of one document's phrasing; this is the class. **T17 is the ordering sibling** — the same family of instruction (constrain which of the proposed topics may run, or in what order) reaching the same nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Fold into T17's proposed ARBITRATION gap exception rather than fixing separately — both are "the document constrains the topic set it just described, and the schema expresses no such constraint". Gapping it is the cheap half and can be done from the prompt; expressing it — a mutual-exclusivity group, a tie-break — is a schema change and a product decision. Note that on both documents the analyst quoted the constraining sentence INSIDE the affected topics' `sourceQuote`s while saying nothing about it, so the words are reaching the model and the omission is downstream of reading.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| T13 | R010                   | 08             | **actioned**                                      | Doc 08 run 2's `routing_analysis` failed hard: _"not valid against the schema after one retry (invalid at: topics.6.questionKeys.0, topics.8.questionKeys.0)"_. Those are Protection Needs and Vulnerability, and their first question keys — minted by the **extractor** on the same ingest — are 70 and 78 characters. `analysis-schema.ts:77` validates `questionKeys` entries at `.max(TOPIC_KEY_MAX_LENGTH)` = **64** (`scope/types.ts:203`); `extraction-schema.ts:50` mints keys as `z.string().min(1)` with **no maximum**. The analyst echoed back real keys for real questions and was rejected for it. The retry cannot help — satisfying the schema means shortening a key, and a shortened key matches no question. The failure is durable (a `failed` row is the "already tried" signal the Topics tab reads) and fail-soft, so the admin sees no routing and no reason. Measured: 1 of 42 corpus versions exceeds 64, and it is the only one of 40 routing analyses to fail; **doc 04 run 2 reached 63 — one character short**. Key length varies run to run on the same document (doc 08's other run peaked at 35), so every long-question instrument plays this lottery on every ingest.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | **Actioned. My "cap at mint" recommendation was the wrong half, and the codebase had already worked out why.** The topics preview route carries the reasoning verbatim — _"Deliberately NOT `TOPIC_KEY_MAX_LENGTH`: that bounds a *topic* key, and these are question and data-slot keys, which nothing bounds at 64… Generous here and permissive downstream"_ — it was simply the only place that had it. New `MEMBER_KEY_MAX_LENGTH = 512` for **references** to keys minted elsewhere; `TOPIC_KEY_MAX_LENGTH` keeps bounding topic keys, which the analyst mints and the `^[a-z0-9_]+$` recipe keeps short. Investigating turned up **two more faces** beyond the analyst failure: the admin's own topic save would 400 on the same version (`schemas.ts` `keyListSchema`), and — quietest and worst — `asKeyList` silently **truncated** to a 64-character prefix, which matches no question, which orphans it, and an orphaned question "can never be asked, and nothing else in the system would ever tell you" (`validate.ts`). Regression tests on all three faces. Whichever way, add a regression test that runs an instrument with >64-character question keys through the analyst — the corpus would not have caught this without doc 08's unusually long prose questions, and did not catch it in nine previous runs.                                            |
| T12 | R009                   | 01             | **actioned**                                      | The scales/matrix repair pass writes a change row via `changeForCorrect` (`_lib/orchestrate-extraction.ts:553`) with `beforeJson`/`afterJson` shaped `{suggestedType, suggestedTypeConfig}` and **no `key`**, and `targetEntityId` null. Two consequences, both verified: (1) the row that changed the question names no question, while the `infer_type` row it contradicts stays `status='applied'`, `supersededAt=null` — doc 01 run 1 shows three questions whose visible rationale reads _"captured as free text to avoid inventing choices"_ against stored `single_choice` slots **with** invented choices; (2) `planInferType` (`extraction-review/planner.ts:454`) reads `beforeJson.type`/`typeConfig`, which this writer never sets, so every repair-originated revert takes the "no prior type recorded" branch and restores **`free_text` with no config**. Confirmed by executing `planRevert` on a repair-shaped change: a `likert`→`matrix` repair plans as _"Restore question to type free_text."_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | **Actioned after R010, once the baseline was banked.** `changeForCorrect` now writes the `key` its sibling `changeForMerge` always did, and the canonical `type`/`typeConfig` spelling the planner reads. `planInferType` also accepts the legacy `suggestedType`/`suggestedTypeConfig` shape, because those rows are already stored and a revert that quietly loses a `likert` is worse than one that fails — a read-path fix, not a backfill. Verified by execution: a repair-shaped `likert`→`matrix` change now plans as _"Restore question to type likert"_ from either spelling, and a genuinely empty `beforeJson` still falls back to `free_text` as documented. Original note: **it was deliberately not fixed inside R009**, which existed to establish a single-build baseline across seven documents. It touches no scored dimension (the routing chain never reads these rows), so fixing it cannot invalidate the 95.7%. The fix is small and in two places: give `changeForCorrect` the `key` its sibling `changeForMerge` already writes, and settle the field names so writer and planner agree (`type`/`typeConfig` is what the planner and every other change type use). Worth a regression test that round-trips a repair through `planRevert` — the mismatch is exactly the kind a type-checker cannot see, because both sides are `unknown`-shaped JSON. |
...
| --------------- | ---------- | ----------------- | ---------------- | --------- | --------------- | -------------- | ------------------- | ------------------------------------------------------- |
| R012 (partial)  | 2026-08-26 | `f99a07c1e`       | `openai/gpt-5.4` | _n/a_     | not run         | _n/a_ (1 of 3) | 1 (doc 07)          | partial — doc 07 only; 6→7/10, critical failure stands  |
| R011 (partial)  | 2026-08-26 | `f99a07c1e`       | `openai/gpt-5.4` | _n/a_     | not run         | _n/a_ (1 of 3) | 0                   | partial — doc 10 only; critical failure cleared, 4→5/10 |
