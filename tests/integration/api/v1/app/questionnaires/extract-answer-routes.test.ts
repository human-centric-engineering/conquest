...
beforeEach(() => {
  ...
  vi.mocked(isFeatureEnabled).mockImplementation((flag) =>
    Promise.resolve(
      flag === APP_QUESTIONNAIRES_FLAG || flag === APP_QUESTIONNAIRES_ANSWER_EXTRACTION_FLAG
    )
  );
...
describe('gate order + auth', () => {
  ...
  it('404s when the answer-extraction sub-flag is off (after auth, no dispatch)', async () => {
