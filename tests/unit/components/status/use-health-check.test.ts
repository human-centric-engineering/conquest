...
describe('useHealthCheck', () => {
  ...
  describe('background-tab gating', () => {
    /**
     * Drive the tab's visibility and fire the corresponding event.
     *
     * Sets BOTH `document.hidden` and `document.visibilityState`. The browser
     * keeps them in step, but happy-dom does not derive one from the other, and
     * since the Sunrise 0.8.0 sync the polling lives in the shared
     * `useAutoRefresh`, which reads `document.hidden`. Stubbing only
     * `visibilityState` left the hook seeing a visible tab, so it refreshed on
     * "hide" instead of pausing.
     */
    ...
    // `restoreAllMocks` undoes the spy but not the defineProperty, so a hidden
    // tab would leak into every test that ran after this block.
    afterEach(() => {
