/**
 * Unit tests for AppAnalyseGlossaryTermsCapability.execute() (P16).
 *
 ...
 * tests establish (`generate-data-slots.test.ts`): every error branch gets its own outcome code,
 * the agent binding actually reaches the resolver, and cost metadata is threaded.
 *
 ...
 *
 * `redactProvenance` is covered separately in `analyse-glossary-terms-redaction.test.ts`.
 */
...
const { AppAnalyseGlossaryTermsCapability } =
  await import('@/lib/app/questionnaire/capabilities/analyse-glossary-terms');
...
describe('execute — happy path', () => {
  ...
  it('threads the resolved provider/model and versionId into the cost log', async () => {
    await capability.execute(ARGS, CONTEXT);
    expect(logCost).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        metadata: expect.objectContaining({
          capability: 'app_analyse_glossary_terms',
          versionId: 'ver-1',
        }),
      })
    );
  ...
  it('sends the questions and goal to the completion as a built prompt', async () => {
    ...
    expect(rendered).toContain('How connected do you feel to your higher self?');
    expect(rendered).toContain('Assess spiritual development');
  });
...
describe('execute — error branches', () => {
  ...
  it('returns glossary_analysis_failed when the completion throws', async () => {
    (runStructuredCompletion as Mock).mockRejectedValue(new Error('schema invalid after retry'));

    const res = await capability.execute(ARGS, CONTEXT);
    expect(res.success).toBe(false);
    expect(res.error?.code).toBe('glossary_analysis_failed');
