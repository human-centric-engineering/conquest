 ...
 *    via agent-resolver.ts (routing tier);
...
describe('app-questionnaire/089-scope-candidacy-agent seed', () => {
  it('upserts the candidacy-check agent provider-agnostic, private, app-owned, budget-capped', async () => {
    ...
    // Empty strings → dynamic resolution via agent-resolver.ts (routing tier).
