...
const prismaMock = vi.hoisted(() => ({
  ...
  // Conditional Topics (P17): `replaceDataSlots` now attaches the slots it just wrote to the version's
  // topics, so the transaction reads and updates topic rows. Defaults to "no topics", which is the
  // state of every version that never opted in — and makes the attach pass a no-op for every test
  // below that is not about it.
...
beforeEach(() => {
  vi.clearAllMocks();
  // Default to "this version has no topics" — the state of every version that never opted into
  // Conditional Topics, which makes the attach pass inside `replaceDataSlots` a no-op for every test
...
/**
 * Conditional Topics (P17): saving data slots gives them a home among the version's topics.
 *
 * The ordering defect this closes is structural, not a slip. Topics are seeded during INGEST, and
 * ingest creates no data slots — those are generated by a separate admin step afterwards. So every
 * seeded topic is born with `dataSlotKeys: []`, and nothing else ever revisits membership: a later
 * structure-replace skips sections that are already covered, and the generator does not know topics
 * exist. Without this pass the two halves of the instrument would never meet, and switching adaptive
 * scope on would leave the conversation with no data slot in scope to target.
 */
describe('replaceDataSlots — attaching the slots to topics', () => {
  const INPUT_SLOTS: DataSlotInput[] = [
    { name: 'Onboarding ease', description: 'D1', theme: 'T1', questionKeys: ['q1', 'q2'] },
    { name: 'Blocker', description: 'D2', theme: 'T1', questionKeys: ['q3'] },
  ];
  ...
  beforeEach(() => {
    ...
    prismaMock.appDataSlot.deleteMany.mockResolvedValue({ count: 0 });
    prismaMock.appDataSlotDraft.deleteMany.mockResolvedValue({ count: 0 });
    prismaMock.appDataSlot.create
      .mockResolvedValueOnce({ id: 'new-slot-1' })
      .mockResolvedValueOnce({ id: 'new-slot-2' });
    prismaMock.appDataSlotQuestion.createMany.mockResolvedValue({ count: 2 });
    // Read twice now: once by the attach pass (inside the tx), once by the closing loadDataSlots.
    prismaMock.appDataSlot.findMany.mockResolvedValue([
      makeDbRow({
        id: 'new-slot-1',
        key: 'onboarding_ease',
  ...
  it('writes each slot onto the topic owning most of its questions', async () => {
    prismaMock.appQuestionnaireTopic.findMany.mockResolvedValueOnce([
      {
        id: 't1',
        key: 'opening',
        ordinal: 0,
      ...
      },
      {
        ...
        key: 'blockers',
        ordinal: 1,
        members: { questionKeys: ['q3'], dataSlotKeys: [] },
      },
    ]);

    await replaceDataSlots('v1', INPUT_SLOTS);

    const updates = (prismaMock.appQuestionnaireTopic.update as Mock).mock.calls.map((c) => ({
      id: c[0].where.id,
      dataSlotKeys: c[0].data.members.dataSlotKeys,
    }));
    expect(updates).toEqual([
      { id: 't1', dataSlotKeys: ['onboarding_ease'] },
      { id: 't2', dataSlotKeys: ['blocker'] },
    ]);
  });

  it('preserves the questions a topic already held', async () => {
    prismaMock.appQuestionnaireTopic.findMany.mockResolvedValueOnce([
      {
        id: 't1',
        key: 'opening',
    ...
    const written = (prismaMock.appQuestionnaireTopic.update as Mock).mock.calls[0]?.[0]?.data
      .members;
    expect(written.questionKeys).toEqual(['q1', 'q2']);
  });

  it('leaves an admin’s own placement alone rather than relocating it', async () => {
    // `onboarding_ease` maps to q1+q2, which `opening` owns — but the admin put it on `blockers`.
    // Re-saving the slots must not quietly undo that.
    prismaMock.appQuestionnaireTopic.findMany.mockResolvedValueOnce([
      {
        id: 't1',
        key: 'opening',
      ...
      },
      {
        ...
        key: 'blockers',
        ordinal: 1,
        members: { questionKeys: ['q3'], dataSlotKeys: ['onboarding_ease'] },
      },
    ]);
    ...
    // Only `blocker` needed a home; `onboarding_ease` stayed where it was put.
    expect(updates).toEqual([{ id: 't2', dataSlotKeys: ['onboarding_ease', 'blocker'] }]);
  });

  it('writes nothing when the version has no topics — the overwhelmingly common case', async () => {
    prismaMock.appQuestionnaireTopic.findMany.mockResolvedValueOnce([]);
  ...
  it('writes nothing on a re-save, because every slot already has a home', async () => {
    prismaMock.appQuestionnaireTopic.findMany.mockResolvedValueOnce([
      {
        id: 't1',
        key: 'opening',
        ordinal: 0,
        members: { questionKeys: ['q1', 'q2'], dataSlotKeys: ['onboarding_ease'] },
      },
      {
        id: 't2',
        key: 'blockers',
        ordinal: 1,
        members: { questionKeys: ['q3'], dataSlotKeys: ['blocker'] },
      },
    ]);
